...
- FAISS was chosen over Chroma to keep the dependency surface smaller and the retrieval path easier to explain.
- Chunking is character-based rather than token-based because it is easier to read and discuss in an interview.
- The answer generation layer is pluggable so the project still runs locally without requiring cloud credentials.
- A minimum similarity score filter is used so the tool can say when the documents do not support an answer.
